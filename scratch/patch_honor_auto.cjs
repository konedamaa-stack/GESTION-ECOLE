...
// The pattern to replace starts with the container div and ends at the closing small tag
const startPattern = `<div style={{marginTop: '16px', marginBottom: '24px', padding: '16px', background: '#f8fafc', borderRadius: '8px', border: '1px dashed #cbd5e1'}}>`;
...
if (startIndex !== -1 && endIndex !== -1) {
  const newBlock = `<div style={{marginTop: '16px', marginBottom: '24px', padding: '16px', background: '#f8fafc', borderRadius: '8px', border: '1px dashed #cbd5e1'}}>
                        ...
                        <div style={{display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap'}}>
                          ...
                          <button className="btn btn-primary" onClick={(e) => {
                            ...
                            const classGrades = gradesData.filter(g => classEvalIds.includes(g.evaluation_id) && g.student_id === selectedStudent.id && g.score !== null);
  ...
  console.log("Patched correctly for automatic merit!");
} else {
  console.log("Failed to find block to patch.", {
