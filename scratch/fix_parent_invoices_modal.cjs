                    ...
                    {parentInvoices.length > 0 ? (
                      <div className="table-responsive">
                        <table className="table" style={{width: '100%', marginBottom: '20px', borderCollapse: 'collapse'}}>
                          ...
                          <tfoot>
                            <tr>
                              <th colSpan={4} style={{textAlign: 'right', padding: '16px 8px', borderTop: '2px solid var(--border-color)', fontSize: '1.1rem'}}>Reste Total :</th>
                              <th colSpan={2} style={{textAlign: 'left', padding: '16px 16px', borderTop: '2px solid var(--border-color)', fontSize: '1.2rem', color: 'var(--error-color)'}}>
                                {formatNum(parentInvoices.reduce((sum, inv) => {
                                  if (inv.status === 'Payée') return sum;
                                  return sum + (Number(inv.amount) - Number(inv.paid_amount || 0));
                                }, 0))} F
                              </th>
                            </tr>
                          </tfoot>
