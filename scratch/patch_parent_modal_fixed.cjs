...
// 1. Update fetchParents to include IDs (if not already done by my previous git restore)
...
// 2. Add handleRemoveChild function right after fetchParents
...
`;
// Insert after fetchParents
const fetchParentsBlock = `const fetchParents = async () => {
    const { data } = await supabase.from('parents').select('*, student_parents(student_id, parent_id, students(id, first_name, last_name))').eq('school_id', currentSchoolId || '');
    if (data) setParentsData(data);
  };`;
content = content.replace(fetchParentsBlock, fetchParentsBlock + '\n' + removeChildFunc);
...
const oldTableChildCol = `<td style={{padding: '16px 0'}}>
...
const newTableChildCol = `<td style={{padding: '16px 0'}}>
                  {row.student_parents?.length > 0 ? (
                    <div style={{display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap'}}>
                      <span className="badge badge-primary">{row.student_parents.length} {row.student_parents.length > 1 ? 'élèves' : 'élève'}</span>
