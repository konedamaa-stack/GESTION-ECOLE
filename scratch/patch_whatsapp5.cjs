...
const regex = /<span className=\{\`badge \$\{selectedStudent\.status === 'Inscrit' \? 'badge-success' : 'badge-warning'\}\`} style=\{\{marginTop: '8px', display: 'inline-block'\}\}>\s*\{selectedStudent\.status\}\s*<\/span>/m;

const newBlock = `<div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginTop: '8px' }}>
...
if (regex.test(content)) {
  ...
  console.log("WhatsApp regex patch successful!!!");
} else {
