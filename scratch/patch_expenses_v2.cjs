...
// 2. Add fetchExpenses after fetchEmployees (line ~859 based on earlier, or let's place it before fetchSettings)
...
// 4. Handlers (handleAddExpense, handleDeleteExpense)
  ...
  code = code.replace(
    ...
    try {
      if (editEntity) {
        ...
        const { error } = await supabase
          .from('expenses')
          .insert([{ school_id: currentSchool.id, category, amount: parseFloat(amount), payment_date: date, description }]);
        if (error) throw error;
      }
      await fetchExpenses();
      closeModal();
...
if (!code.includes(`activeTab === 'depenses'`)) {
  code = code.replace(
    /<li className=\{`nav-item \$\{activeTab === 'rh' \? 'active' : ''\}`\} onClick=\{\(\) => \{ setActiveTab\('rh'\); setIsMobileMenuOpen\(false\); \}\}>\s*<span className="nav-icon">👥<\/span>\s*<span className="nav-text">\{t\('sidebar\.rh', 'RH'\)\}<\/span>\s*<\/li>/,
    `$&
...
if (!code.includes(`activeModal === 'expense'`)) {
  code = code.replace(
    /\{activeModal === 'employee_payment' && \([\s\S]*?<\/div>\s*\)\s*\}/,
    `$&
        {activeModal === 'expense' && (
