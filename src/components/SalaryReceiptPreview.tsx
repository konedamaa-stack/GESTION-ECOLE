  ...
  return (
    <div className="receipt-preview-container" style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '12px', marginBottom: '15px' }}>
        <span>Versé à :</span>
        <span style={{ fontWeight: 'bold', fontSize: '13px' }}>{(employee.first_name + ' ' + employee.last_name).toUpperCase()}</span>
        <span style={{ fontSize: '11px' }}>Poste : {employee.role || 'N/A'}</span>
      </div>
      ...
      <div style={{ marginTop: '35px', display: 'flex', justifyContent: 'space-between' }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontStyle: 'italic', color: '#000', fontSize: '11px' }}>Signature Employé</div>
        </div>
        <div style={{ textAlign: 'center' }}>
