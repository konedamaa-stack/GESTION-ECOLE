...
export const ExpenseReceiptPreview: React.FC<ExpenseReceiptPreviewProps> = ({ expense, schoolInfo }) => {
  useEffect(() => {
    ...
    <div className="receipt-container" style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #e5e7eb', paddingBottom: '20px', marginBottom: '30px' }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <h2 style={{ margin: '0 0 8px 0', fontSize: '28px', color: '#3b82f6', textTransform: 'uppercase', letterSpacing: '1px' }}>REÇU DÉPENSE</h2>
          <p style={{ margin: '0 0 4px 0', fontWeight: 'bold', fontSize: '16px' }}>N° {expense.id.split('-')[0].toUpperCase()}</p>
          <p style={{ margin: '0', color: '#4b5563' }}>Date: {formatDate(expense.payment_date || new Date().toISOString())}</p>
        </div>
      </div>

      <div style={{ marginBottom: '40px' }}>
        <div style={{ backgroundColor: '#f8fafc', padding: '20px', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '30px' }}>
          <h3 style={{ margin: '0 0 16px 0', fontSize: '16px', color: '#64748b', textTransform: 'uppercase' }}>Détails de la Dépense</h3>
          
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div>
              <p style={{ margin: '0 0 8px 0', color: '#64748b', fontSize: '14px' }}>Catégorie</p>
              <p style={{ margin: '0', fontWeight: 'bold', fontSize: '16px', color: '#0f172a' }}>{expense.category}</p>
            </div>
          </div>
        </div>
          ...
          <tbody>
            <tr>
              <td style={{ padding: '16px', borderBottom: '1px solid #e2e8f0', color: '#334155' }}>
                {expense.description || ("Paiement pour " + expense.category)}
              </td>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <div style={{ width: '300px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderTop: '2px solid #cbd5e1', borderBottom: '2px solid #cbd5e1', marginTop: '10px' }}>
              ...
              <span style={{ fontWeight: 'bold', fontSize: '18px', color: '#3b82f6' }}>{formatCurrency(expense.amount)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: '50px', display: 'flex', justifyContent: 'space-between', color: '#64748b', fontSize: '14px' }}>
        <div>
          <p style={{ margin: '0 0 40px 0' }}>Le Comptable / Administrateur</p>
      ...
      <div style={{ marginTop: '40px', textAlign: 'center', color: '#94a3b8', fontSize: '12px', borderTop: '1px solid #e2e8f0', paddingTop: '20px' }}>
        <p style={{ margin: '0' }}>Ce reçu est généré automatiquement par le système de gestion {(schoolInfo?.school_name || schoolInfo?.name) || ''}.</p>
      </div>
